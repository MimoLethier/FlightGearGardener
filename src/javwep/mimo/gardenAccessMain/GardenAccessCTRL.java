...
*		and possibly for that reason too, I see FlightGearGardener also as a (very humble !) celebration of James Goslin's marvelously clever, robust,
...
public class GardenAccessCTRL implements Initializable
 {
   ...
   // Browsing...
   private final int maxBrowseLevel = 5;
   ...
   private ObservableList<String> browseDirData, browseFileData;
   ...
   @FXML private  Button goBrowseBTN, loadItBTN;
   @FXML private  ListView airportsView, browseDirView, browseFileView;
...
// wwwwwwwwwwwwwwwwwwwwwwwwwww
   ...
   public void getAlive(Chooser theAcknowledger, Stage theStage) throws Exception
	...
	// 5. Other initializations :  "Selecting" background handler, Airports info listview ....
	...
	browseFileData = FXCollections.observableArrayList();
	browseFileView.setItems(browseFileData);

	// Open the "Selection"  and the "Download" Clerc
	try {
	...
	headLine1.setText("Regional focus: None selected, yet.");
...
// wwwwwwwwwwwwwwwwwwwwwwwwwww
   ...
   private void onTabSelection( MouseEvent event )
	...
	if ( laScene.getSelectionModel().getSelectedItem() == selectionTAB) {
	   postIt("Please pick your centre of interest...");
	   return;
	}
   }
...
// wwwwwwwwwwwwwwwwwwwwwwwwwww
   ...
   private void onBackToSelectingTAB (ActionEvent event)
	...
	postIt("Please pick a new centre of interrest, or Exit ...");
   ...
   private void onApplyZoomRequest (ActionEvent event)
	...
	postIt("Please select your Terrasync repository, or go back to Selecting...");
   ...
   private void onFindTerrasyncRequest(ActionEvent event)
	...
	try {
	   ...
	   if ( errCnt == -1 ) {
		postIt("Found NO POPULATED tile in this Folder for the selected Region... Select Tiles to load, or return.");
		headLine2.setText("Region currently NOT populated yet.");
	   } else {
		if ( errCnt == 0 ) {
		   postIt("Found loaded tiles in this Folder for the selected Region... Try SYNC, Clear, or go Back...");
   ...
   private void onApplySelectionRequest (ActionEvent event)
	...
	switch ( downloadClerc.getRunningAction() ) {
	   ...
	   case CLEAR : {
		...
		if ( ! yesChoosen("Do you confirm DELETING content for selected Tiles ?") ) {
		   applySelectionBTN.setDisable(false);
		   clearSelectionBTN.setDisable(false);
		   return;
		}
   ...
   private void onGoBrowseRequest (ActionEvent event)
    {
	try {
	   headLine2.setText("Browsing Terrasync Root Page...");
	   headLine3.setText("Retained Synchronisation Server: " + downloadClerc.openSyncSession());
   ...
   private void onBrowseDirViewClicked (MouseEvent event)
    {
	if ( currentBrowseLevel++ >= maxBrowseLevel) { return; }
	...
	if ( ! browseRequestGOTResults() ) {
	   ...
	   headLine2.setText("Browsing Terrasync at " + sourceUrlSTB.toString());
	}
   ...
   // <+> <+> <+>
   @FXML
   private void onBrowseFileViewClicked (MouseEvent event)
    {
	loadItBTN.setDisable(false);
   }


   // Load the last selected file
   // <+> <+> <+>
   @FXML
   private void onLoadItRequest (ActionEvent event)
	...
	try {
	   URL sourceURL = new URL(sourceUrlSTB.toString() + (String) browseFileView.getSelectionModel().getSelectedItem());
	   ...
	   postIt("The selected file has been saved to the current Terrasync folder...");
...
// wwwwwwwwwwwwwwwwwwwwwwwwwww
   ...
   private boolean browseRequestGOTResults()
    {
	...
	for (String aLine : browseList) {
	   ...
	   if ( matchClerc.matches() ) {
		...
		matchClerc = AdHoc.TILE_TERR_PATTERN.matcher(aLine);
		if ( matchClerc.matches() ) {
		   browseFileData.add( matchClerc.group(2));
		}
	   }
	}
	if (  browseFileData.size() <= 0  ) {
	   browseFileView.setMouseTransparent(true);
	   ...
	   Collections.sort(browseFileData);
	   browseFileView.setMouseTransparent(false);
	   browseFolderLBL.setText("... " + sourceUrlSTB.toString().substring(browseLevelsLength[0]));
	}
	return true;
   }
